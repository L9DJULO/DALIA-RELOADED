...
import { fetchLCUStatus, connectLCU } from '../services/api';
...
const useLCUStore = create((set, get) => ({
  ...
  /**
   * Fetch current LCU status from backend
   */
  fetchStatus: async () => {
    try {
      const data = await fetchLCUStatus();
      set({
  ...
  /**
   * Manually trigger connection to LCU
   */
  connect: async () => {
    try {
