...
# Roles that can threaten each position (ordered: primary → secondary)
THREAT_ROLES = {
    "top":     ["top", "jungle"],
    "jungle":  ["jungle", "mid"],
...
# Weight multipliers per (my_role → threat_role):
#   same lane = 1.6x priority; bot↔support = 1.2x; others = 0.7x
LANE_FOCUS: dict = {
    "top":     {"top": 1.6, "jungle": 0.7},
    "jungle":  {"jungle": 1.6, "mid": 0.7},
    "mid":     {"mid": 1.6, "jungle": 0.7, "support": 0.6},
    "bot":     {"bot": 1.6, "support": 1.2, "jungle": 0.6},
    "support": {"support": 1.6, "bot": 1.2, "jungle": 0.6},
}
...
class BanRecommender:
    ...
    async def recommend_bans(
        ...
        for champ in all_champs:
            ...
            if counter_score > 0 and pool_entries:  # only show if user has a pool
                score += counter_score * 0.40
                if counter_score >= 60:
                    reasons.append(f"Contre fort votre pool ({my_role})")
                elif counter_score >= 30:
                    reasons.append(f"Contre modéré votre pool ({my_role})")
            elif counter_score > 0:
                score += counter_score * 0.40  # still counts toward score, just no reason label

            # ─── 2. Meta strength — same lane prioritized (30%) ───
            # Same-lane champion gets a strong bonus; bot ↔ support also boosted.
            lane_weights = LANE_FOCUS.get(my_role, {})
            meta_score = 0.0
            meta_role = ""
            for role in threat_roles:
                raw_ms = self.meta.score(champ.id, role)
                weight = lane_weights.get(role, 0.7)
                weighted_ms = raw_ms * weight
                if weighted_ms > meta_score:
                    meta_score = weighted_ms
                    meta_role = role
            ...
            # ─── 3. Community ban rate — also lane-weighted (15%) ───
            ban_rate_score = 0.0
            for role in threat_roles:
                stats = self.db.get_stats(champ.id, role)
                if stats and stats.ban_rate > 0:
                    br_s = min(stats.ban_rate * 4, 100)  # 25% BR → 100 score
                    # Same-lane ban rate counts more
                    br_s_weighted = br_s * lane_weights.get(role, 0.7)
                    if br_s_weighted > ban_rate_score:
                        ban_rate_score = br_s_weighted

            if ban_rate_score > 0:
                score += ban_rate_score * 0.15
                if ban_rate_score >= 80:
                    reasons.append("Très fréquemment banni (même lane)")
                elif ban_rate_score >= 50:
                    reasons.append("Fréquemment banni")
