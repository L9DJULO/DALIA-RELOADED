...
class DraftNet(nn.Module):
    ...
    def get_raw_embedding(self, champion_id: int) -> Optional[List[float]]:
        """Return the base embedding vector for a champion (without role projection)."""
        if champion_id <= 0 or champion_id >= NUM_CHAMPIONS:
            return None
        self.eval()
        with torch.no_grad():
            t = torch.tensor([champion_id], dtype=torch.long)
            emb = self.champion_embed(t).squeeze(0)  # (embed_dim,)
            return emb.tolist()

    def get_role_embedding(self, champion_id: int, role_idx: int) -> Optional[List[float]]:
        """Return the role-projected embedding for a champion in a specific role.

        Args:
            champion_id: Riot champion ID
            role_idx: 0=top, 1=jg, 2=mid, 3=bot, 4=sup
        """
        if champion_id <= 0 or champion_id >= NUM_CHAMPIONS or role_idx < 0 or role_idx > 4:
            return None
        self.eval()
        with torch.no_grad():
            t = torch.tensor([champion_id], dtype=torch.long)
            raw = self.champion_embed(t).squeeze(0)  # (embed_dim,)
            projected = self.role_proj[role_idx](raw)  # (embed_dim,)
            return projected.tolist()

    def get_all_embeddings(self, champion_ids: List[int], role_idx: int = 2) -> dict:
        """Return role-projected embeddings for a list of champion IDs.

        Args:
            champion_ids: list of Riot champion IDs
            role_idx: role context for projection (default mid)

        Returns: {champion_id: [float, ...]} for each valid champion
        """
        self.eval()
        result = {}
        with torch.no_grad():
            for cid in champion_ids:
                emb = self.get_role_embedding(cid, role_idx)
                if emb is not None:
                    result[cid] = emb
        return result
