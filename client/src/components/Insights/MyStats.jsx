...
/* Big stat card */
function StatCard({ icon: Icon, iconColor, label, value, sub, valueColor = 'text-[var(--text-primary)]' }) {
  return (
    <div className="panel p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[11px] text-[var(--text-secondary)] uppercase tracking-wider font-medium">{label}</span>
      </div>
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-surface-elevated/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-[var(--text-primary)] truncate">{champ.champion_name}</div>
        <div className="text-[11px] text-[var(--text-muted)]">{champ.count} partie{champ.count > 1 ? 's' : ''}</div>
      </div>
          ...
          return (
            <div key={role} className="flex items-center gap-3">
              <div className="w-20 flex items-center gap-1.5">
                <RoleIcon role={role} size={16} className="text-[var(--text-secondary)]" />
                <span className="text-xs text-[var(--text-secondary)] font-medium capitalize">{role}</span>
              </div>
              ...
              <div className="flex items-center gap-2 w-28 justify-end">
                <span className="text-[11px] text-[var(--text-muted)] tabular-nums">{data.games}G</span>
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <div className="text-sm text-[var(--text-secondary)]">Chargement de tes stats…</div>
        </div>
      </div>
    );
  }
  ...
  if (!stats || stats.total_games === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <div className="text-sm font-medium text-[var(--text-secondary)] mb-1">Pas encore de données</div>
    ...
    <div className="space-y-4">
      ...
      {recentForm.length > 0 && (
        <div className="panel p-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="ml-3 text-xs text-[var(--text-secondary)]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {stats.most_picked?.length > 0 && (
          <div className="panel p-4">
            <div className="text-[11px] text-[var(--text-secondary)] uppercase tracking-wider font-medium mb-2">
      ...
      <div className="panel p-4">
        <div className="flex items-center gap-2 mb-3">
          <Link2 size={14} className={summoner?.puuid ? 'text-emerald-400' : 'text-[var(--text-muted)]'} />
        ...
        {!summoner?.puuid ? (
          <div className="text-center py-6">
            <Unlink size={24} className="text-[var(--text-muted)] mx-auto mb-2" />
          ...
          <div className="text-center py-6">
            ...
            <div className="text-xs text-[var(--text-secondary)]">Chargement des stats ranked…</div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                ...
                <div className="text-[10px] text-[var(--text-muted)]">Parties</div>
              </div>
              <div className="text-center">
                <div className={`text-lg font-bold tabular-nums ${
              ...
              <div className="text-center">
                ...
                <div className="text-[10px] text-[var(--text-muted)]">Champions</div>
              </div>
            </div>
            ...
            {personalStats.role_distribution && (
              <div className="flex gap-2">
                {Object.entries(personalStats.role_distribution)
                  ...
                  .sort((a, b) => b[1] - a[1])
                  .map(([role, count]) => (
                    <div key={role} className="flex items-center gap-1 text-[11px] text-[var(--text-secondary)]">
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-surface-elevated/50">
                      <img
                        ...
                        alt={champInfo.name}
                        className="w-7 h-7 rounded border border-[var(--border-subtle)]"
                        loading="lazy"
                      />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-medium text-[var(--text-primary)] truncate">
                          {champInfo.name}
                          <span className="text-[var(--text-muted)] ml-1 capitalize">{role}</span>
                        </div>
                      </div>
                      <div className="text-[11px] text-[var(--text-muted)] tabular-nums">
                        {stats.games}G
                      </div>
                      <div className="text-[11px] text-[var(--text-secondary)] tabular-nums w-12 text-right">
                        {stats.kda} KDA
                      </div>
