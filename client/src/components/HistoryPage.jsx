...
import { fetchHistory, updateHistoryResult } from '../services/api';
...
export default function HistoryPage({ onReplay }) {
  ...
  async function result(id, value) {
    try { const entry = await updateHistoryResult(id, value); if (alive.current) setEntries(items => items.map(e => e.id === id ? entry : e)); }
  ...
  return <main className="workshop history-page">
    ...
    {entries.map(entry => <article key={entry.id}>
      ...
      <p>Patch {entry.patch || 'inconnu'} · {entry.timeline?.length || 0} étapes · {entry.result || 'résultat non renseigné'}</p>
      <div className="controls">
        <button disabled={!entry.timeline?.length} onClick={() => replay({ schema_version: 1, steps: entry.timeline })}>Rejouer</button>
        <button disabled={!entry.timeline?.length} onClick={() => downloadReplay({ schema_version: 1, steps: entry.timeline })}>Exporter</button>
      ...
      {!entry.timeline?.length && <p className="muted">Ancienne entrée : les étapes n’étaient pas enregistrées.</p>}
    </article>)}
  </main>;
}
